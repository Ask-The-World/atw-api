...
use crate::conf_vars::{self, ConfVars, get_conf_vars};
...
pub async fn get_client_with_config(config: conf_vars::ConfVars) -> mongodb::error::Result<Client> {
    let client = Client::with_uri_str(&format!("mongodb://{}:{}@{}:{}/", config.db_user, config.db_password, config.db_server, config.db_port)[..]).await?;
    return Ok(client)
}
...
pub async fn ping_server_slow(config: conf_vars::ConfVars) ->  mongodb::error::Result<String> {

    let client = get_client_with_config(config).await?;
    // Ping the server to see if you can connect to the cluster
    client
        .database("admin")
        .run_command(doc! {"ping": 1}, None)
